...
if($imagePost) {
    ...
    $sql1 = "UPDATE professor SET name1='$name', expertise='$exp', background='$bg', 
    aoi='$aoi', projects='$projects', research='$research', links='$links' WHERE email='$email'";
    $res1 = mysqli_query($conn, $sql1);
    echo "<br/>$res1";
    if (mysqli_affected_rows($conn) > 0) {                                                                
        echo "<br/>Info updated.";                                                
        ...
        echo "<br/>Info not updated.";
        $sql2 = "INSERT INTO professor (name1, expertise, background, aoi, projects, research, links) 
        VALUES ('$name', '$exp', '$bg', '$aoi', '$projects', '$research', '$links')";
        $res2 = mysqli_query($conn, $sql2);
        if (mysqli_affected_rows($conn) > 0) {
            echo "<br/>Info Inserted.";
        } else {
